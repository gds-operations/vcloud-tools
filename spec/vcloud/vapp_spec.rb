  ...
  describe Vcloud::Vapp do
    ...
    context "power on" do
      context "successful power on" do
        before(:each) do
          ...
          Vcloud::FogInterface.stub(:new).and_return(@mock_fog_interface)
        end
    ...
    context "provisioning a vapp" do
      ...
      it "should return a vapp if it already exists" do
        existing_vapp = double(:mocked_vapp)
        ...
        Vcloud::FogInterface.should_receive(:new).and_return(@mock_fog_interface)

        Vcloud.logger.should_receive(:info)
        actual_vapp = Vcloud::Vapp.provision config
